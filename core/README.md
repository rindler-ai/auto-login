...
credentials and TOTP seeds **at rest only on the user's device** and, when the
...
| `totp` | On-device RFC 6238 TOTP + `otpauth://` QR parsing. The seed stays on the device; only the generated code is relayed. RFC 6238 Appendix B vectors are tested. |
...
No durable secret ever transits. TOTP seeds and mailbox tokens never leave the
device — the device generates the code locally and relays only the code. Only the
